...
from workflow.nodes.audio_render_planner_v2.application.stage import AudioRenderPlannerV2Module


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="audio render planner v2 CLI")
    p.add_argument("--prompt", default="")
    p.add_argument("--input-json", default="")
    p.add_argument("--run-root", default="")
    p.add_argument("--planner-model-root", default="")
    return p.parse_args()
...
def main() -> None:
    ...
    run_root = str(payload.get("run_root", "")).strip() or f".result/workflow/nodes/audio-render-planner-v2/{_run_id()}"
    payload["run_root"] = run_root
    ctx = PipelineContext(run_id=_run_id(), run_root=run_root, area="workflow", pipeline="audio")

    result = AudioRenderPlannerV2Module().process(ctx, payload)
