"""준비된 MoMask로 원본 모션을 보존하고 SMPL 피팅 준비 상태를 확인한다.

Depth 렌더 구현은 아직 연결되지 않았다. 누락된 SMPL 모델을 다른 메쉬로
대체하지 않으며 생성 완료된 관절 모션은 실패 후에도 재사용하도록 보존한다.
"""
...
WORKFLOW_REPOSITORY_ROOT = Path(__file__).resolve().parents[1]
...
MOTION_RUN_DIRECTORY = WORKFLOW_REPOSITORY_ROOT / '.result/workflow/runs/motion-depth-attempt-v2'
MOTION_ASSET_DIRECTORY = WORKFLOW_REPOSITORY_ROOT / '.result/workflow/reusable/motions/walk-travel/v1'
...
SMPL_NEUTRAL_MODEL = WORKFLOW_REPOSITORY_ROOT / '.model/motion-depth/smpl/SMPL_NEUTRAL.pkl'
...
def execute_motion_attempt():
    ...
    prepared_model_record = json.loads((MOTION_RUN_DIRECTORY / 'prepared.json').read_text())
    ...
    print('96프레임 걷기 관절 모션 추론 시작', flush=True)
    ...
    motion_asset_record = {
        ...
        'prepared_manifest_sha256': calculate_file_digest(MOTION_RUN_DIRECTORY / 'prepared.json'),
    ...
    if not SMPL_NEUTRAL_MODEL.is_file():
        raise FileNotFoundError(f'SMPL 피팅용 중립형 모델 누락: {SMPL_NEUTRAL_MODEL}; 모션은 보존됨. SMPL/Depth 대체 생성 없음')
    raise NotImplementedError('SMPL 모델 존재 확인 완료. 피팅·Depth 렌더 실행기는 아직 연결되지 않음')
...
def run_logged_attempt():
    ...
    try:
        with subprocess.Popen([sys.executable, '-u', str(Path(__file__).resolve()), '--worker'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True) as motion_process_handle:
...
if __name__ == '__main__':
    if sys.argv[1:] == ['--worker']:
        ...
        raise SystemExit('지원하지 않는 실행 인자')
    else:
        run_logged_attempt()
