...
- 현재 통합 대상은 `momask`, `qwen-2512`, `qwen-2511`, `character-animation`이다. MoMask와 캐릭터 애니메이션은 로컬 작업 서비스를 통해 서버 없이 CLI 실행이 가능하고 Qwen 두 종류는 실행 중인 관리 서버 API를 사용한다. ANNY·작가 에이전트 등 미통합 기능까지 완료된 것으로 기술하지 않는다. 신규 관리 기능과 기존 기능의 통합 작업에는 이 구조를 적용한다.
