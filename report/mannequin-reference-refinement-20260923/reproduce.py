...
def execute_reproduction_run():
    ...
    execution_result_record = {'status':'running', 'started_at':datetime.now(ZoneInfo('Asia/Seoul')).isoformat(), 'archive_files_verified':verified_file_count, 'python':sys.version, 'blender':importlib.metadata.version('bpy'), 'numpy':np.__version__, 'scope':'accepted mesh -> profile sculpt -> hip sockets and thigh shells -> surface partitions -> weights -> rig -> GLB -> 25-frame validation; no inference or rendering'}
    try:
        shutil.copytree(ARCHIVE_SOURCE_ROOT / 'snapshot/inputs', execution_output_root / 'inputs')
        for source_file_name in ['sculpt_source.py','face_details.py','build_rig.py','build_parts.py','refine_weights.py','validate_rig.py','validate_export.py']:
            shutil.copy2(ARCHIVE_SOURCE_ROOT / 'snapshot' / source_file_name, execution_output_root / source_file_name)
        ...
        for stage_script_name in ['sculpt_source.py','build_rig.py','validate_rig.py','validate_export.py']:
