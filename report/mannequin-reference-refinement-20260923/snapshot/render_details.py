...
try:
 ...
 render_detail_view('detail-face',(0,-.05,1.77),(0,-6,.15),.46)
 render_detail_view('detail-face-side',(0,-.02,1.77),(6,-2,.15),.46)
 render_detail_view('detail-shoulder',(0.17,0,1.40),(3,-6,1),.48)
 render_detail_view('detail-hip',(0,0,.985),(3,-6,1),.62)
 render_detail_view('detail-knee',(.111,.015,.56),(3,-6,1),.40)
 ...
 for pose_bone_value in rig_object_value.pose.bones:
  pose_bone_value.rotation_quaternion=(1,0,0,0)
 rig_object_value.pose.bones['joint-04'].rotation_quaternion=Quaternion(Vector((0,0,1)),math.radians(-35))
 bpy.context.view_layer.update()
 render_detail_view('hip-abduction',(0,0,1),(0,-6,.8),1.15)
 for pose_bone_value in rig_object_value.pose.bones:
  pose_bone_value.rotation_quaternion=(1,0,0,0)
 rig_object_value.pose.bones['joint-04'].rotation_quaternion=Quaternion(Vector((1,0,0)),math.radians(-70))
 rig_object_value.pose.bones['joint-07'].rotation_quaternion=Quaternion(Vector((1,0,0)),math.radians(75))
 bpy.context.view_layer.update()
 render_detail_view('hip-flexion',(0,0,.97),(5,-3,1),1.3)
 write_trace_message('complete','관절 확대, 약 35도 벌림 및 약 70도 굽힘 관찰 렌더 완료')
