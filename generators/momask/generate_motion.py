...
def execute_motion_attempt():
    ...
    prepared_model_record = json.loads(PREPARED_MODEL_MANIFEST.read_text())
    ...
    motion_asset_record = {
        ...
        'prepared_manifest_sha256': calculate_file_digest(PREPARED_MODEL_MANIFEST),
...
if __name__ == '__main__':
    ...
    MOTION_ASSET_DIRECTORY = MOTION_RUN_DIRECTORY / 'motion'
    PREPARED_MODEL_MANIFEST = parsed_argument_values.prepared_manifest.resolve()
    if parsed_argument_values.worker:
        ...
        if not PREPARED_MODEL_MANIFEST.is_file():
            raise FileNotFoundError(f'MoMask 모델 준비 기록 누락: {PREPARED_MODEL_MANIFEST}')
