...
def render(motion_path, output_root, sample_indices=None, frame_duration_ms=300):
    ...
    if len(sample_indices) != 8 or sample_indices.min() < 0 or sample_indices.max() >= len(joints): raise ValueError("유효한 8개 샘플 인덱스가 필요합니다.")
    ...
    manifest={"schema_version":1,"kind":"momask-joint-pose-sheets","source_frames":len(joints),"sample_indices":sample_indices.tolist(),"directions":list(DIRECTIONS),"sheet_size":[2048,1024],"cell_size":[512,512],"frame_duration_ms":frame_duration_ms}
