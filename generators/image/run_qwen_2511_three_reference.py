...
def execute_three_reference_worker():
    ...
    with current_lock_path.open('a') as current_lock_handle:
        try:
            ...
            execute_pose_generation(enable_text_only_generation=not current_request_record['references'],trial_output_root=current_job_root,prompt_text_value=current_request_record['prompt'],character_image_path=current_job_root/'reference-1.png',pose_reference_path=current_job_root/'reference-2.png',additional_reference_paths=(current_job_root/'reference-3.png',),pose_reference_kind='rig',selected_reference_order='standing-first',selected_output_width=current_request_record['width'],selected_output_height=current_request_record['height'],**resolve_reference_settings(current_request_record['steps']),prompt_source_record={'kind':'management-three-reference','reference_order':[1,2,3]})
