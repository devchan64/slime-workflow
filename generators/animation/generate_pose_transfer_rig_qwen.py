#!/usr/bin/env python3
"""캐릭터 아이덴티와 리그 참조만 사용하는 Qwen 포즈 전환기."""
import argparse
import hashlib
...
def generate_pose_transfer_rig_qwen_frame(output_directory, prompt_file_path):
    ...
    return execute_pose_generation(
        ...
        pose_reference_path=Path(output_directory) / 'rig-reference.png',
        pose_reference_kind='rig',
        selected_reference_order='standing-first',
        selected_inference_steps=4,
        prompt_source_record={'kind': 'qwen-lightning-rig-reference', 'sha256': hashlib.sha256(prompt_source_text.encode()).hexdigest()},
...
def run_pose_transfer_rig_qwen_command():
    ...
    return generate_pose_transfer_rig_qwen_frame(parsed_arguments.output_dir.resolve(), parsed_arguments.prompt_file)


if __name__ == '__main__':
    run_pose_transfer_rig_qwen_command()
