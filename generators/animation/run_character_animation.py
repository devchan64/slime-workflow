...
def generate_character_frame(generation_job_path,current_frame_index):
    ...
    selected_inference_steps=generation_request_record.get('steps',4)
    selected_lightning_mode=selected_inference_steps==4
    selected_anny_mode=generation_request_record['source']=='anny'
    execute_pose_generation(selected_output_width=generation_request_record.get('resolution',512),selected_output_height=generation_request_record.get('resolution',512),trial_output_root=frame_output_directory,prompt_text_value=selected_prompt_text,character_image_path=input_reference_paths['character'],pose_reference_path=input_reference_paths['pose'],pose_reference_kind='rig' if selected_anny_mode else 'openpose',selected_inference_steps=selected_inference_steps,prompt_source_record={'kind':'registered-character-animation','motion':generation_request_record['motion'],'sha256':selected_prompt_hash,'words':selected_prompt_words,'direction':source_frame_record['direction']},enable_anypose_adapter=selected_anny_mode,enable_lightning_adapter=selected_lightning_mode,enable_standalone_lightning_adapter=selected_lightning_mode and not selected_anny_mode)
...
def generate_character_animation(generation_job_path):
    ...
    with generation_gpu_lock_path.open('a') as generation_gpu_lock_handle:
        ...
        for current_frame_index,source_frame_record in enumerate(generation_request_record['frames']):
            ...
            write_record_atomically(generation_job_path/'progress.json',{'completed':current_frame_index+1,'total':len(generation_request_record['frames'])})
        write_record_atomically(generation_job_path/'result.json',{'resolution':generation_request_record.get('resolution',512),'steps':generation_request_record.get('steps',4),'lightning':generation_request_record.get('steps',4)==4,'fps':generation_request_record['fps'],'frames':generation_result_frames,'motion':generation_request_record['motion'],'source':generation_request_record['source'],'sampling':generation_request_record['sampling'],'speed':generation_request_record.get('speed',1),'target_fps':generation_request_record.get('target_fps'),'source_fps':generation_request_record.get('source_fps'),'frame_step':generation_request_record.get('frame_step',1),'source_frame_numbers':{direction_name_value:[frame_record_value['frame'] for frame_record_value in generation_request_record['frames'] if frame_record_value['direction']==direction_name_value] for direction_name_value in generation_request_record['directions']}})
