...
def generate_character_animation(generation_job_path):
    ...
    with generation_gpu_lock_path.open('a') as generation_gpu_lock_handle:
        ...
        write_record_atomically(generation_job_path/'result.json',{'steps':generation_request_record.get('steps',4),'lightning':generation_request_record.get('steps',4)==4,'fps':generation_request_record['fps'],'frames':generation_result_frames,'motion':generation_request_record['motion'],'source':generation_request_record['source'],'sampling':generation_request_record['sampling'],'target_fps':generation_request_record.get('target_fps'),'source_fps':generation_request_record.get('source_fps'),'frame_step':generation_request_record.get('frame_step',1),'source_frame_numbers':{direction_name_value:[frame_record_value['frame'] for frame_record_value in generation_request_record['frames'] if frame_record_value['direction']==direction_name_value] for direction_name_value in generation_request_record['directions']}})
