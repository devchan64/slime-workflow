...
도서 편집 화면(`/worldbuilding/library`, 통합 메뉴 `#worldbuilding-library`)에서 도서를 선택하고 **1~4단계 한 번에 실행**을 누르면 아래 단계를 순차 실행한다. CLI의 개별 단계 명령도 유지한다.
...
3. **문서 재구성**(`document-reconstruction`): 현재 원본의 문서·줄 순서를 보존한 결정적 배치안을 출력한다. 원본 변경이나 모델 추론은 실행하지 않는다.
4. **문서 정리**(`document-cleanup`): 현재 원본을 다시 수집해 읽기용 Markdown 검수본을 만든다. 문서당 한 장으로 원문 순서를 보존하며 동일한 제목의 다른 파일도 합치지 않는다. 도입 README와 Markdown 본문을 먼저, YAML 자료를 뒤에 둔다.

전체 실행은 `book_edit_stage_name: all-stages`로 등록한다. 1단계 요약을 묶음별로 읽어 2단계 목차를 구성하고, 3단계는 목차 기록과 원문 순서 배치안을 남기며 4단계에서 배치 무결성을 검증한다. 의미에 따른 문단 재배치나 원문 수정은 자동 실행하지 않는다. CLI 개별 단계 실행은 독립적이다. 재실행 결과는 새 작업 경로에 보관한다. 원본 파일 변경은 별도의 파일 변경 미리보기와 반영 명령으로 처리한다. 정리 단계는 원본을 수정하지 않는다.

전체 실행은 `jobs/<작업 ID>/pipeline.yaml`에 단계별 시작·종료 시각과 상태를 저장하고, `stages/01-document-summary/`부터 `stages/04-document-cleanup/`까지 요청·입력 연결·결과·로그를 보관한다. 단계 실패 시 남은 단계는 실행하지 않는다. 원본 해시는 단계 사이에 대조하며 변경되면 중단한다. 같은 도서에 진행 중인 작업이 있으면 중복 등록하지 않는다. 관리 서버 재시작으로 중단된 작업은 실패로 표시하며 완료한 기록은 남는다.

화면의 **단계별 기록·경로 조회**에서 절대 경로와 각 단계의 요청·상태·로그·결과를 확인한다. 전체 산출물 선택기로 모델 응답과 완성 문서의 장도 조회한다. 로그는 큰 파일의 마지막 부분, 기타 기록은 앞부분을 최대 128,000바이트까지 표시하며 전체 파일 경로를 안내한다. 작업 이력은 웹 작업과 CLI 작업을 포함해 20개씩 조회한다. 읽기 API는 해당 작업 안의 Markdown·YAML·로그만 허용하며 경로 이탈·심볼릭 링크를 차단한다.

정리 결과는 정리 단계 경로의 `completed-book/`에 저장한다. 최상위 제목은 장마다 하나로 유지하고 원본의 추가 최상위 제목은 하위 제목으로 표시한다. YAML은 언어가 지정된 코드 블록으로 표시한다. `paragraphs.yaml`에 줄바꿈까지 보존한 원문 블록을 기록하며 누락·중복·해시 불일치·실행 중 원본 변경은 실패한다. `manifest.yaml` 버전 2는 본문 문단 수, 원문 블록 수, YAML 자료 수를 구분한다.

포함 문서의 상대 링크는 장 파일로 다시 연결한다. 참조식 링크·절 앵커를 유지하며 코드 예시의 링크는 바꾸지 않는다. 범위 밖 링크는 원본 작업 공간의 파일로 연결하고 이식성 경고를 기록한다. 원본부터 존재하지 않는 파일·절은 `WARN`과 `quality_warnings`에 남긴다. 지원하지 않는 링크 문법 때문에 출력 목적지가 맞지 않으면 즉시 실패하며 깨진 판본을 완료로 표시하지 않는다.

관리도구의 **도서 편집**에서 **세계관 / 시스템 설계** 중 하나를 선택하고 **선택한 도서 편집**을 누른다. 등록한 루트 폴더의 문서를 자동으로 불러오며 문서의 첫 최상위 제목·파일명으로 주제를 분류하되 문서 내부 순서는 유지한다. 자동 분류를 확인하고 장 이름을 수정하거나 문단 순서를 옮긴 뒤 **이 배치로 웹 도서 생성**을 누른다. 동일 장 이름은 한 장으로 모이며 장 순서는 첫 등장 순서, 장 안에서는 배치 순서를 따른다.
