...
도서 편집 화면에서 세계관 또는 시스템 설계를 선택하고 편집 지시를 입력한 뒤 **AI 자동 편집 시작**을 누른다. 고정 Qwen 로컬 GPU 모델이 문서 목록으로 통합 목차를 설계하고, 원문 문단을 입력 예산에 맞춰 묶어 읽으면서 장·순서·색인어·대상 파일을 결정한다. 수동 배치 조정은 보조 기능으로 유지한다.
