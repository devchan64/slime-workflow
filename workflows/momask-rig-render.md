...
스트레칭의 `parallel-transport-v3`는 매 프레임 기본 팔 방향과 비교하던 회전을 직전 프레임 방향에서 누적하는 방식으로 바꾼다. 팔을 머리 위로 올려 기본 방향과 반대가 될 때 회전축이 급변하는 문제를 줄인다. 위팔·아래팔의 원본 방향을 유지하며 기존 추가 축 회전 제한도 유지한다. 키프레임 쿼터니언은 직전 프레임과 같은 부호 반구로 저장해 보간 중 불필요한 회전을 방지한다.

검증 원본 `2026-09-25_09-43-53-f7ab7fbf`, 전체 120프레임: 오른쪽 위팔 로컬 회전의 최대 프레임 변화 약 172.7°→13.2°, 오른쪽 아래팔 약 131.4°→12.2°, 왼쪽 아래팔 약 80.6°→16.5°. 양팔 쿼터니언 부호 뒤집힘은 없어졌다. 40·45·50·77프레임 진단 렌더를 확인했다. 발 회전 보정은 변경하지 않는다. 기존 결과는 유지하고 새 렌더부터 적용한다.

## 재사용 스트레칭 에셋

사용자 선택 이력 `2026-09-25_09-56-57-42fc2f95`를 `assets/motion-sheet/momask-stretch-v1`에 등록했다. 이후 스트레칭 포즈 재사용 시 `generators/animation/config/default_stretch_motion.yaml`을 기준으로 원본 모션·ANNY 리그·120프레임×4방향 포즈 맵을 참조한다. 프롬프트 재추론으로 등록된 에셋을 대체하지 않는다.
