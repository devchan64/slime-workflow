...
원본 `joints` 위치 채널을 사용한다. 독립적인 두 방향이 선언된 구간은
위치로 방향 프레임을 복원하고, 단일 방향 구간은 기준 본에서 첫 방향을 맞춘 뒤
프레임 사이의 최소 회전을 누적한다. 팔·다리·발·쇄골·목에 같은 단일 방향 규칙을
사용한다. 골반·척추는 프로필에 선언한 좌우 관절도 사용한다. 필수 두 방향이
일직선이 되어 방향 프레임을 만들 수 없으면 원인을 표시하고 실패한다.
단일 방향의 축 비틀림은 원본에서 복원한 정보가 아닌 연속 운반 규약이다.
...
방식 식별자 `position-profile-transport-v2`를 남긴다. `review-metrics.json`은
구간별 관측 방식·계산 방향 오차·실제 리그 방향 오차·프레임 간 회전을 기록한다.
