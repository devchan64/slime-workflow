...
걷기 이미지젠의 동작 참조 원본은 셰이딩을 보강한 리그 8프레임 시트다. 호출당 최대 4프레임만 생성하며, 1–4번·5–8번 리그를 각각 2×2 참조로 나누어 생성한 뒤 최종 4×2 걷기 시트로 통합한다. `assets/pose-sheets/five-head-walk-8f/v1/`의 OpenPose 시트는 비교 자료로 유지한다. `generators/animation/build_walk_pose_sheets.py`가 렌더 실행 폴더의 8프레임 포즈를 묶고 출처·파일 해시를 기록한다.
...
[스탠딩 시트 제작 절차](workflows/character-standing-sheet.md)는 승인된 2×2 베이스라인 시트 전체를 참조하여 방향별 4프레임의 4×1 시트 4장을 생성한다. 호출당 최대 4프레임 기준을 적용하고 입력 베이스라인 셀과 출력 방향의 대응을 명시한다. 정식 에셋 교체는 사용자 채택 후 수행한다.
