...
def build_animation_tools(output):
    ...
    records.append({'id':'character-animation','label':'캐릭터 애니메이션 생성기','path':'/character-animation/','anchorEditor':False,'category':'animation-tool','description':'모션 에셋 · 캐릭터 레퍼런스 · 방향 선택 · 생성 이력과 재생'})
    records.append({'id':'sprite-editor','label':'스프라이트 정규화 편집기','path':'/character-animation/sprite-editor','anchorEditor':False,'category':'animation-tool','description':'생성 ID · 프레임 정렬 · 중심·바닥·머리 가이드 · 프레임 재생 · 시트 내보내기'})
    return records
