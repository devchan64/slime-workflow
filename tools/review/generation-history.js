...
async function refreshGenerationHistory(){
 try{
  ...
  for(const currentHistoryRecord of currentHistoryPayload.records.slice((currentHistoryPage-1)*historyPageSize,currentHistoryPage*historyPageSize)){
   ...
   currentHistorySummary.textContent=currentHistoryRecord.created_at+' · '+({running:'생성 중',completed:'완료',cancelled:'취소됨',failed:'실패',missing:'파일 없음'}[currentHistoryRecord.status.status]||currentHistoryRecord.status.status)+' · '+(currentHistoryRecord.request.motion?[currentHistoryRecord.request.motion,currentHistoryRecord.request.character,currentHistoryRecord.request.source,currentHistoryRecord.request.directions.join(', '),(currentHistoryRecord.request.frame_step||1)+'프레임 간격'].join(' · '):currentHistoryRecord.request.attributes?'ANNY 속성':(currentHistoryRecord.request.steps||4)+'스텝');
