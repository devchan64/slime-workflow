    ...
    def handle_image_request(self, current_http_handler):
        ...
        try:
            ...
            if current_http_handler.command == 'POST':
                ...
                send_response_data(200,Path(__file__).with_name('generation-studio.css').read_bytes(),'text/css; charset=utf-8')
            elif current_url_path == self.route_prefix_value+'/progress-ui.js':
                send_response_data(200,Path(__file__).with_name('generation-progress.js').read_bytes(),'text/javascript; charset=utf-8')
            elif current_url_path == self.route_prefix_value+'/history-ui.js':
                send_response_data(200,Path(__file__).with_name('generation-history.js').read_bytes(),'text/javascript; charset=utf-8')
            elif current_url_path in (self.route_prefix_value,self.route_prefix_value+'/'):
                send_response_data(200,Path(__file__).with_name('three-reference-generation.html' if self.three_reference_mode else 'image-generation.html').read_bytes(),'text/html; charset=utf-8')
            else:
