...
async function refreshGenerationHistory(){
 ...
 try{
  ...
  for(const currentHistoryRecord of visibleHistoryRecords.slice((currentHistoryPage-1)*historyPageSize,currentHistoryPage*historyPageSize)){
   ...
   historyInputSummary.textContent=currentHistoryRecord.request.motion?[currentHistoryRecord.request.motion,currentHistoryRecord.request.character,currentHistoryRecord.request.source==='openpose'?'OpenPose':'ANNY',(currentHistoryRecord.request.directions||[]).map(directionNameValue=>historyDirectionLabels[directionNameValue]||directionNameValue).join(' · '),(currentHistoryRecord.request.target_fps?'타겟 '+currentHistoryRecord.request.target_fps+' FPS':(currentHistoryRecord.request.frame_step||1)+'프레임 간격'),(currentHistoryRecord.request.steps||4)+'스텝'].join(' / '):currentHistoryRecord.request.attributes?'ANNY · 캐릭터 속성 렌더링':(currentHistoryRecord.request.steps||4)+'스텝'+(currentHistoryRecord.request.width?' · '+currentHistoryRecord.request.width+' × '+currentHistoryRecord.request.height:'');
