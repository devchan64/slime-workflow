    ...
    with gr.Blocks(title='타일 에셋 생성기') as blocks_value:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                read_history_page,history_output_values=build_generation_history_view(execute_tile_gateway,server_base_address,'타일 생성 이력과 결과·참조 사본·로그 파일을 삭제합니다. 생성 중에는 초기화할 수 없습니다.',lambda record:restore_tile_inputs(record,server_base_address),[tile_value,prompt_value,width_value,step_value,seed_value,base_value,style_value,reference_style_value,*reference_image_controls,status_value])
