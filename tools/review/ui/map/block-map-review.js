...
function splitWallFloors(currentFaceRecord){
 ...
 for(let currentFloorIndex=Math.floor(minimumFaceHeight/60);currentFloorIndex<Math.ceil(maximumFaceHeight/60);currentFloorIndex++){
  let clippedFaceVertices=currentFaceRecord.vertices;
  for(const [currentClipHeight,currentKeepAbove] of [[currentFloorIndex*60,true],[(currentFloorIndex+1)*60,false]]){
...
function selectWallTexture(currentFaceRecord){
 ...
 const roofBaseHeight=Math.min(...currentBuildingRecord.blocks.filter(currentBlockRecord=>currentBlockRecord.material==='roof').map(currentBlockRecord=>currentBlockRecord.layer*60+(currentBlockRecord.offsetHeight||0)));
