...
class QwenCommandsTest(unittest.TestCase):
    def test_both_generation_routes(self):
        for service_name_value,route_prefix_value,default_seed_value in [('qwen-2512','/image-generation',251204),('qwen-2511','/image-generation-2511',10107)]:
            with patch.object(COMMAND_MODULE_VALUE,'call_management_api',return_value={'id':'test','status':'running'}) as api_call_handle, contextlib.redirect_stdout(io.StringIO()):
                self.assertEqual(COMMAND_MODULE_VALUE.execute_qwen_command(service_name_value,['generate','--prompt','한국어 프롬프트','--detach']),0)
    ...
    def test_reference_order_and_prompt_file(self):
        with tempfile.TemporaryDirectory() as temporary_directory_name:
            ...
            with patch.object(COMMAND_MODULE_VALUE,'call_management_api',return_value={'id':'test'}) as api_call_handle, contextlib.redirect_stdout(io.StringIO()):
                COMMAND_MODULE_VALUE.execute_qwen_command('qwen-2511',['generate','--prompt-file',str(prompt_file_path),'--reference',str(reference_file_path),'--seed','42','--steps','30','--detach'])
    ...
    def test_failed_generation_exit_status(self):
        with patch.object(COMMAND_MODULE_VALUE,'call_management_api',side_effect=[{'id':'test'},{'status':'failed','error':'실패'}]), contextlib.redirect_stdout(io.StringIO()):
            self.assertEqual(COMMAND_MODULE_VALUE.execute_qwen_command('qwen-2512',['generate','--prompt','test']),1)
    ...
    def test_help_does_not_connect(self):
        with patch.object(COMMAND_MODULE_VALUE,'call_management_api') as api_call_handle, contextlib.redirect_stdout(io.StringIO()):
            with self.assertRaises(SystemExit) as exit_context_value:
                COMMAND_MODULE_VALUE.execute_qwen_command('qwen-2511',['generate','--help'])
    ...
    def test_invalid_id_does_not_connect(self):
        with patch.object(COMMAND_MODULE_VALUE,'call_management_api') as api_call_handle:
            with self.assertRaises(ValueError):
                COMMAND_MODULE_VALUE.execute_qwen_command('qwen-2511',['status','../../etc/passwd'])
            api_call_handle.assert_not_called()
