...
def build_execution_logs():
    with gr.Accordion('실행 로그 · 펼쳐서 확인',open=False) as log_panel_element:
        ...
        log_output_element=gr.Textbox(label='작업을 선택하세요',lines=18,max_lines=30,interactive=False,autoscroll=True,buttons=['copy'],elem_id='management-execution-log',elem_classes=['management-log-output'])
    ...
    log_panel_element.expand(fn=None,js=log_scroll_script,queue=False)
