...
async function renderMotionAssetFrame(){
 ...
 try{
  ...
  assetElementLookup('asset-frame-label').textContent=`${requestedFramePosition+1} / ${assetMotionRecord.frames} · ${assetMotionRecord.fps} FPS`;
...
assetElementLookup('asset-frame-play').onclick=()=>{
 ...
 const advanceMotionAssetFrame=async()=>{
  ...
  if(await renderMotionAssetFrame()&&assetPlaybackRunning&&playbackSelectionVersion===assetSelectionVersion)assetPlaybackTimer=setTimeout(advanceMotionAssetFrame,Math.max(0,1000/assetMotionRecord.fps-(performance.now()-frameStartTimestamp)));
 };
 assetPlaybackTimer=setTimeout(advanceMotionAssetFrame,1000/assetMotionRecord.fps);
};
