...
def execute_pose_generation(*, trial_output_root, prompt_text_value,
                            ...
                            prompt_source_record=None, enable_anypose_adapter=False):
    ...
    if enable_anypose_adapter and (pose_reference_kind != 'rig' or selected_reference_order != 'standing-first' or selected_inference_steps != 4):
        raise ValueError('AnyPose 경로는 리그·캐릭터 우선·4스텝으로 고정합니다.')
    selected_true_cfg_scale = 1.0 if enable_anypose_adapter else 4.0
    ...
    try:
        ...
        trial_result_record = {'status':'completed','model_id':'Qwen/Qwen-Image-Edit-2511','revision':FIXED_MODEL_REVISION,'size':[512,512],'reference_vae_size':[512,512],'reference_condition_size':[384,384],'steps':selected_inference_steps,'seed':FIXED_GENERATOR_SEED,'true_cfg_scale':selected_true_cfg_scale,'guidance_scale':1.0,'lightning_lora':enable_anypose_adapter,'dtype':'bfloat16','execution_device':'cuda','weight_offload':'sequential_cpu_offload','torch_version':torch.__version__,'diffusers_version':diffusers.__version__,'elapsed_seconds':round(time.monotonic()-run_started_time,2),'prompt_sha256':hashlib.sha256(prompt_text_value.encode()).hexdigest(),'input_order':[input_file_path.name for input_file_path in input_image_paths],'input_sha256':{input_file_path.name:hashlib.sha256(input_file_path.read_bytes()).hexdigest() for input_file_path in input_image_paths},'quality_warnings':['실험 결과의 최종 품질 승인이 필요합니다.'],'output':'result.png'}
        trial_result_record.update({'pose_reference_kind': pose_reference_kind, 'reference_order': selected_reference_order, 'prompt_source': prompt_source_record, 'adapters': resolved_adapter_records, 'execution_preset': 'anypose-lightning-v1' if enable_anypose_adapter else 'base-v1', 'input_references': [{'role': input_image_role, 'path': str(input_file_path), 'sha256': hashlib.sha256(input_file_path.read_bytes()).hexdigest()} for input_image_role, input_file_path in zip(input_image_roles, input_image_paths)]})
