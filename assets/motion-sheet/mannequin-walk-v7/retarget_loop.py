...
scene_render_value=bpy.context.scene;scene_render_value.frame_start=1;scene_render_value.frame_end=len(source_joint_frames);scene_render_value.render.fps=20
...
for current_frame_index,current_joint_points in enumerate(source_joint_frames):
 ...
 for current_pose_bone in rig_object_value.pose.bones:
  ...
  if current_bone_name=='root':target_rotation_value=pelvis_rotation_value@rest_bone_rotations[current_bone_name]
   ...
   if current_bone_name.startswith('foot'):
    # 발 관절 정의가 다른 리그 사이에는 절대 벡터가 아니라 중립 대비 회전을 전달한다.
    source_rest_vector=source_motion_bundle['rest'][current_source_end]-source_motion_bundle['rest'][current_source_start]
    source_rest_vector=Vector((float(source_rest_vector[0]),float(-source_rest_vector[2]),float(source_rest_vector[1])))
    target_rotation_value=source_rest_vector.rotation_difference(current_motion_vector)@rest_bone_rotations[current_bone_name]
   else:
