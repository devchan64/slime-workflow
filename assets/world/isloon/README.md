...
정식 맵은 `maps/iseulon.yaml`이며, 세계 확장 설정의 24×24 안전 도시 배치(청록 샘터, 꽃 화단, 길드 회관·서점·여관·대장간·시장, 서·동·남 출구)를 따른다. 이슬온의 길드 회관·대장간·시장(상점)은 대리석으로 지어진 건물이며, 해당 프리팹은 `marble_large_window_wall`과 `marble_small_door`를 사용한다. 서점과 여관은 목재건물로 정의했으며, 목재 벽·문 타일은 향후 생성 후 프리팹 참조를 교체한다. 최종 타일 이미지는 `slime-frontend/src/assets/world/isloon/`에 둔다.
